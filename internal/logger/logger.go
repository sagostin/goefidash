...
func (l *Logger) buildRow(ts time.Time, e *ecu.DataFrame, g *gps.Data) []string {
	...
	if e != nil {
		...
		row[3] = fmt.Sprintf("%d", e.TPS)
		row[4] = fmt.Sprintf("%.1f", e.AFR)
		row[5] = fmt.Sprintf("%.3f", e.Lambda)
		row[6] = fmt.Sprintf("%d", e.CLT)
		row[7] = fmt.Sprintf("%d", e.IAT)
		row[8] = fmt.Sprintf("%d", e.Advance)
		row[9] = fmt.Sprintf("%.1f", e.Battery)
		row[10] = fmt.Sprintf("%.3f", e.PW1)
		row[11] = fmt.Sprintf("%.3f", e.PW2)
		row[12] = fmt.Sprintf("%.1f", e.DutyCycle)
		row[13] = fmt.Sprintf("%d", e.VE)
		...
		row[22] = fmt.Sprintf("%d", e.WUECorrection)
		row[23] = fmt.Sprintf("%d", e.Corrections)
		row[24] = boolStr(e.Status4&0x08 != 0)
		row[25] = boolStr(e.Spark&0x04 != 0)
		row[26] = boolStr(e.Engine&0x01 != 0)
	}
